...
from .transformer_temporal import TemporalTransformerBlock
...
class CameraPoseEncoder(nn.Module):
    ...
    # [b, dof, f, h, w] -> [b, 4, f, h / 8, w / 8]
    def forward(self, x):
        # unshuffle
        bs = x.shape[0]                                 # [2, 6, 16, 256, 384]
        x = rearrange(x, "b c f h w -> (b f) c h w")    # [32, 6, 256, 384]
        x = self.unshuffle(x)                           # [32, 384, 32, 48] => ([32, 6 * 8 * 8, 256 / 8, 384 / 8])
        # extract features
        features = []
        x = self.encoder_conv_in(x)                     # [32, 320, 32, 48]
